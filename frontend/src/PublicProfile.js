...
// Profile with Card ID verification (new URL format: /u/{username}/{cardId})
const PublicProfileWithCard = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        // Verify card_id is valid and matches this profile
        const response = await axios.get(`${API}/public/${username}/card/${cardId}`);
        setData(response.data);
      } catch (err) {
        if (err.response?.status === 404) {
          setError("not_found");
        } else if (err.response?.status === 403) {
          setError("card_mismatch");
        } else {
          setError("error");
        }
      } finally {
  ...
  // Error state - Profile or Card not found
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background to-muted/50 px-4" data-testid="profile-error">
        <Card className="w-full max-w-md border-0 shadow-xl">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-muted-foreground mb-6">
              {error === "card_mismatch" 
                ? "Cette carte n'est pas associée à ce profil."
                : "Ce profil n'existe pas ou le code de carte est invalide."}
            </p>
  ...
  return (
    <div className="min-h-screen gradient-bg-subtle" data-testid="public-profile-with-card">
      <div className="max-w-md mx-auto px-4 py-8">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
...
export { ProfileByUserId, PublicProfileWithCard };
